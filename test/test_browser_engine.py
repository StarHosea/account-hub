...
class TrialCheckTest(unittest.TestCase):
    def setUp(self):
        from services.register import trial_check
        self.trial_check = trial_check

    def _patch_cfg(self, cfg: dict):
        return mock.patch.object(self.trial_check, "config", SimpleNamespace(trial_check=cfg))

    def test_disabled_is_eligible(self):
        with self._patch_cfg({"enabled": False, "base_url": "", "api_key": ""}):
            r = self.trial_check.check_eligibility("tok", "a@b.com")
        self.assertTrue(r["eligible"])
        self.assertEqual(r["reason"], "trial_check_disabled")

    def test_empty_base_url_is_eligible(self):
        with self._patch_cfg({"enabled": True, "base_url": "", "api_key": "k"}):
            r = self.trial_check.check_eligibility("tok")
        self.assertTrue(r["eligible"])

    def _mock_post(self, payload: dict, status: int = 200):
        resp = SimpleNamespace(status_code=status, text=json.dumps(payload), json=lambda: payload)
        return mock.patch("curl_cffi.requests.post", return_value=resp)

    def test_eligible_true(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://cha.nerver.cc", "api_key": "k"}):
            with self._mock_post({"token_ok": True, "eligible": True, "reason": "ok"}):
                r = self.trial_check.check_eligibility("tok", "a@b.com")
        self.assertTrue(r["eligible"])

    def test_ineligible(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://cha.nerver.cc", "api_key": "k"}):
            with self._mock_post({"token_ok": True, "eligible": False, "reason": "账号无试用资格"}):
                r = self.trial_check.check_eligibility("tok", "a@b.com")
        self.assertFalse(r["eligible"])
        self.assertEqual(r["reason"], "账号无试用资格")

    def test_nested_data_ineligible(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://cha.nerver.cc", "api_key": "k"}):
            with self._mock_post({"code": 200, "data": {"token_ok": True, "eligible": False}}):
                r = self.trial_check.check_eligibility("tok")
        self.assertFalse(r["eligible"])

    def test_items_array_eligible(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://cha.nerver.cc", "api_key": "k"}):
            with self._mock_post({"items": [{"token_ok": True, "eligible": True}]}):
                r = self.trial_check.check_eligibility("tok")
        self.assertTrue(r["eligible"])

    def test_upi_ineligible(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://cha.nerver.cc", "api_key": "k"}):
            with self._mock_post({"token_ok": True, "eligible": True, "upi_eligible": False}):
                r = self.trial_check.check_eligibility("tok")
        self.assertFalse(r["eligible"])

    def test_token_invalid_is_unknown(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://cha.nerver.cc", "api_key": "k"}):
            with self._mock_post({"token_ok": False, "reason": "expired"}):
                r = self.trial_check.check_eligibility("tok")
        self.assertIsNone(r["eligible"])  # token 无效 → 未知，fail-open 保号
        self.assertIn("token_invalid", r["reason"])

    def test_base_url_with_path_suffix_is_normalized(self):
        captured = {}

        def _fake_post(url, **kwargs):
            captured["url"] = url
            return SimpleNamespace(status_code=200, text="{}", json=lambda: {"token_ok": True, "eligible": True})

        with self._patch_cfg({"enabled": True, "base_url": "https://cha.nerver.cc/api/v1/check", "api_key": ""}):
            with mock.patch("curl_cffi.requests.post", side_effect=_fake_post):
                r = self.trial_check.check_eligibility("tok")
        self.assertTrue(r["eligible"])
        self.assertEqual(captured["url"], "https://cha.nerver.cc/api/v1/check")

    def test_network_error_fail_open(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://t.example", "api_key": "k"}):
            with mock.patch("curl_cffi.requests.post", side_effect=RuntimeError("boom")):
                r = self.trial_check.check_eligibility("tok")
        self.assertIsNone(r["eligible"])  # 未知 → fail-open（调用方按合格处理，但记录 reason）
        self.assertIn("trial_check_error", r["reason"])

    def test_non_200_unknown(self):
        with self._patch_cfg({"enabled": True, "base_url": "https://t.example", "api_key": "k"}):
            with self._mock_post({"msg": "bad"}, status=500):
                r = self.trial_check.check_eligibility("tok")
        self.assertIsNone(r["eligible"])
