...
const resolvedLocale = String(args.locale || process.env.CLOAK_LOCALE || '').trim();
const page = await session.context.newPage();
if (resolvedLocale) {
  try {
    const cdp = await session.context.newCDPSession(page);
    await cdp.send('Emulation.setLocaleOverride', { locale: resolvedLocale });
  } catch (e) {
    log(`CDP locale 覆盖失败：${e?.message || e}`, 'warn');
  }
}
...
if (resolvedLocale) log(`   locale=${resolvedLocale}（Accept-Language + navigator + CDP override）`);
